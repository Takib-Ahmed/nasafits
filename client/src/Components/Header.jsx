...
to={item.id ? `/details/${item.id}`:'/shop'}
    ...
    SearchTerm != '' &&  <li className="px-3 py-1.5 text-gray-500">No results found</li>
       ...
       className='mobilesearch fixed top-[3rem] w-[100%]  left-0' type='mbsearchbar' location={location} />}
         ...
         <Badge color="white" content={cartedProduct.length} size='sm'  isInvisible={false} shape="circle"    className={`   ${cartedProduct.length>9 && 'p-0.5 px-[0.2rem] '} mainbadge  outline-none border-none me-2.5 sm:me-[0.9rem]  md:me-[0.9rem] lg:me-[1.1rem] mt-1.5  wf text-white bg-black   `}>
